...
function createParticipantCard() {
  ...
  //動態建立 DOM。document.createElement('div') 會在記憶體中建立一個新的 <div> 元素。
...
/*
使用模板字串 (用反引號 \ 包起來) 來產生一大段 HTML。
${index}：這個語法會把 index 變數 (例如 0) 塞進字串裡。 ->  產生了獨一無二的 id
*/
...
form.addEventListener('submit', async (event) => {
  ...
  list.querySelectorAll('input').forEach((input) => { //list.querySelectorAll('input').forEach(...): 把所有動態新增的 input 抓出來，全部跑一次 validateInput。
